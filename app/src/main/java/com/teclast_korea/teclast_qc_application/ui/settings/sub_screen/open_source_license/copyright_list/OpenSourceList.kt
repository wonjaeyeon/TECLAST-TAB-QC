package com.teclast_korea.teclast_qc_application.ui.settings.sub_screen.open_source_license.copyright_list
...
class OSLRepository {
    @Composable
    fun getAllLicenseInfo(): List<OpenSourceLicenseInfo> {

        // Dynamic data
        val generator = LicenseJsonGenerator(LocalContext.current)
        val dynamicLicensesJson = generator.generateJson()
        val dynamicLicenses = Gson().fromJson(dynamicLicensesJson, Array<OpenSourceLicenseInfo>::class.java).toList()
