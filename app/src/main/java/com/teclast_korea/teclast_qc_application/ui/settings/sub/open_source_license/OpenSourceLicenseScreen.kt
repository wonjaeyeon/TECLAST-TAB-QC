package com.teclast_korea.teclast_qc_application.ui.settings.sub.open_source_license
...
import com.teclast_korea.teclast_qc_application.data.oss_license.OSSLicenseInfoRepository
...
fun OpenSourceLicenseScreen(context: Context, navController: NavController) {
    ...
    val OSSLicenseInfoRepository = OSSLicenseInfoRepository()
    val getAllData = OSSLicenseInfoRepository.getAllLicenseInfo()
...
//                ShowLicensesButton_2(LocalContext.current)
            ...
            items(getAllData.size) { index ->
                CustomItem(context = context, OSSLicenseInfo = getAllData[index])
...
@Composable
fun CustomItem(context: Context, OSSLicenseInfo: OSSLicenseInfo) {
    Button(
        onClick = {

            try {
                val intent = Intent(Intent.ACTION_VIEW).apply {
                    data = Uri.parse(OSSLicenseInfo.link)
                }
        ...
        Row(
            ...
            Column(
                ...
                Text(
                    text = OSSLicenseInfo.Name,
                    style = MaterialTheme.typography.h6,
                    color = MaterialTheme.colors.onPrimary
                )
                Text(
                    text = OSSLicenseInfo.License,
                    style = MaterialTheme.typography.body2,
                    color = MaterialTheme.colors.onSurface
                )
                Text(
                    text = OSSLicenseInfo.copyright,
                    style = MaterialTheme.typography.body2,
                    color = MaterialTheme.colors.onSurface
                )
                Text(
                    text = OSSLicenseInfo.link,
                    style = MaterialTheme.typography.body2,
                    color = MaterialTheme.colors.onSurface
                )
            }
